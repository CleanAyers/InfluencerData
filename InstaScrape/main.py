...
async def fetch_all_pages(shortcode: str, session_tuple: Tuple[str, str, str, str], rps: float) -> Tuple[List[str], List[Dict[str, Any]]]:
    ...
    if not HAS_HTTP2:
        print("Warning: h2 package missing; HTTP/2 disabled. Run `pip install \"httpx[http2]\"` to re-enable.")
    async with httpx.AsyncClient(http2=HAS_HTTP2, timeout=httpx.Timeout(20.0, connect=10.0), limits=limits) as client:
